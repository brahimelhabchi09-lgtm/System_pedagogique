...
<head>
  ...
  <title>Create Brief | SHAIMEURCoders</title>
  <!-- Google Fonts -->
  ...
  <div class="flex h-screen flex-col">
    ...
    <div class="flex flex-1 overflow-hidden">
        ...
        <div>
          ...
          <div class="flex flex-col gap-1">
            ...
            <a class="flex items-center gap-3 rounded-lg bg-primary/10 px-4 py-2.5 text-academic-olive transition-all"
              ...
              <span class="text-sm font-bold">Briefs</span>
            </a>
          </div>
        </div>
      </aside>
      ...
      <main class="flex-1 overflow-y-auto p-8 flex justify-center items-start">
        <div
          class="w-full max-w-2xl bg-white dark:bg-slate-900 border border-paper-border rounded-xl shadow-xl overflow-hidden">
          <div class="bg-academic-olive dark:bg-slate-800 px-8 py-6">
          ...
          <form action="/admin/submitBrief" method="POST" class="p-8 space-y-6">
            <input type="hidden" name="author_id" value="1">

            <div class="space-y-1">
              <label class="block text-xs font-black uppercase tracking-widest text-slate-400">Brief Title</label>
            ...
            <div class="space-y-1">
              <label class="block text-xs font-black uppercase tracking-widest text-slate-400">Description /
                Objectives</label>
              <textarea name="description" rows="4" placeholder="Describe the mission..."
            ...
            <div class="grid grid-cols-2 gap-6">
              <div class="space-y-1">
                <label class="block text-xs font-black uppercase tracking-widest text-slate-400">Launch Date</label>
                <input type="date" name="start_date" required
                  class="w-full bg-slate-50 dark:bg-slate-800 border-paper-border rounded-lg py-3 px-4 text-sm font-bold focus:ring-primary focus:border-primary">
              </div>
              <div class="space-y-1">
                <label class="block text-xs font-black uppercase tracking-widest text-slate-400">Deadline</label>
            ...
            <div class="space-y-1">
              <label class="block text-xs font-black uppercase tracking-widest text-slate-400">Associated Sprint
                Cycle</label>
              <select name="sprint_id"
                class="w-full bg-slate-50 dark:bg-slate-800 border-paper-border rounded-lg py-3 px-4 text-sm font-bold focus:ring-primary focus:border-primary">
                <?php foreach ($sprints as $sprint): ?>
                <option value="<?= $sprint->getId() ?>"><?= htmlspecialchars($sprint->getName()) ?></option>
                <?php endforeach; ?>
              </select>
            </div>
