...
<head>
  ...
  <title>Define Skill | SHAIMEURCoders</title>
  <!-- Google Fonts -->
  ...
  <div class="flex h-screen flex-col">
    ...
    <div class="flex flex-1 overflow-hidden">
      ...
      <main class="flex-1 overflow-y-auto p-8 flex justify-center items-start">
        <div
          class="w-full max-w-xl bg-white dark:bg-slate-900 border border-paper-border rounded-xl shadow-xl overflow-hidden">
          <div class="bg-academic-blue px-8 py-6">
          ...
          <form action="/admin/submitSkill" method="POST" class="p-8 space-y-6">
            <div class="grid grid-cols-3 gap-6">
              <div class="col-span-1 space-y-1">
                <label class="block text-xs font-black uppercase tracking-widest text-slate-400">Skill Code</label>
                <input type="text" name="code" required placeholder="C1..."
                  class="w-full bg-slate-50 dark:bg-slate-800 border-paper-border rounded-lg py-3 px-4 text-sm font-bold focus:ring-primary focus:border-primary uppercase">
              </div>
              <div class="col-span-2 space-y-1">
                <label class="block text-xs font-black uppercase tracking-widest text-slate-400">Skill Title</label>
                <input type="text" name="title" required placeholder="e.g. Database Design"
                  class="w-full bg-slate-50 dark:bg-slate-800 border-paper-border rounded-lg py-3 px-4 text-sm font-bold focus:ring-primary focus:border-primary">
              </div>
            </div>

            <div class="space-y-1">
              <label class="block text-xs font-black uppercase tracking-widest text-slate-400">Competency
                Description</label>
              <textarea name="description" rows="3" placeholder="Define what the student should master..."
                class="w-full bg-slate-50 dark:bg-slate-800 border-paper-border rounded-lg py-3 px-4 text-sm font-medium focus:ring-primary focus:border-primary resize-none"></textarea>
            </div>

            <div class="grid grid-cols-2 gap-6">
              <div class="space-y-1">
                <label class="block text-xs font-black uppercase tracking-widest text-slate-400">Assessment
                  Level</label>
                <select name="level"
                  class="w-full bg-slate-50 dark:bg-slate-800 border-paper-border rounded-lg py-3 px-4 text-sm font-bold focus:ring-primary focus:border-primary">
              ...
              <div class="space-y-1">
                <label class="block text-xs font-black uppercase tracking-widest text-slate-400">Contextual
                  Brief</label>
                <select name="brief_id"
                  class="w-full bg-slate-50 dark:bg-slate-800 border-paper-border rounded-lg py-3 px-4 text-sm font-bold focus:ring-primary focus:border-primary">
                  <option value="">-- General Skill --</option>
                  <?php foreach ($briefs as $brief): ?>
                  <option value="<?= $brief->getId() ?>"><?= htmlspecialchars($brief->getTitle()) ?></option>
                  <?php endforeach; ?>
                </select>
              </div>
            </div>

            <div class="pt-6 flex items-center justify-between border-t border-paper-border">
              <button type="button" onclick="window.history.back()"
                class="text-sm font-black text-slate-400 hover:text-slate-600 transition-colors uppercase tracking-widest">
                Cancel
              </button>
              <button type="submit"
