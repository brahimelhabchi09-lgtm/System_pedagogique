...
<head>
  ...
  <title>Manage Classes | SHAIMEURCoders</title>
  <!-- Google Fonts -->
    ...
    <div class="flex flex-1 overflow-hidden">
      ...
      <main class="flex-1 overflow-y-auto p-8">
        <div class="mb-8 flex justify-between items-center">
          <h1 class="text-4xl font-black tracking-tight text-academic-olive dark:text-white">Classes Management</h1>
          <a href="/admin/createClass"
            ...
            <tbody class="divide-y divide-paper-border">
              ...
              <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                <td class="px-6 py-4 text-sm font-bold text-slate-900 dark:text-white">
                  <?= htmlspecialchars($classe->getName()) ?></td>
                <td class="px-6 py-4 text-sm font-medium">
                  <span
                    class="px-3 py-1 bg-primary/20 text-academic-olive rounded-full text-xs font-bold uppercase tracking-tighter">
                    <?= htmlspecialchars($classe->getFormation()) ?>
                  </span>
                </td>
                <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-400 font-medium">
                  <?= htmlspecialchars($classe->getSchoolYear()) ?></td>
                <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-400 font-medium">
                  <?= htmlspecialchars($classe->getGrade()) ?></td>
              </tr>
              <?php endforeach; ?>
            </tbody>
          </table>
